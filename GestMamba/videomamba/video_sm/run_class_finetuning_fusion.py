# python run_class_finetuning_fusion.py --test_best --dist_eval --bf16
import argparse
import datetime
import numpy as np
import time
...
from engines.engine_for_finetuning_fusion import train_one_epoch, validation_one_epoch, final_test, merge
from utils_fusion import NativeScalerWithGradNormCount as NativeScaler
from utils_fusion import multiple_samples_collate
import utils_fusion
import contextlib
...
# os.environ['LOCAL_RANK'] = '0,1,2'

JOB_NAME = 'GestMamba_f32_b5_s123'

OUTPUT_DIR = f"./work_dir/{JOB_NAME}"
LOG_DIR = f"./logs/{JOB_NAME}"
...
def get_args():
    ...
    # Model parameters
    parser.add_argument('--model', default='videomamba_small_fusion', type=str, metavar='MODEL',
                        help='Name of model to train')
    parser.add_argument('--tubelet_size', type=int, default=1)
    parser.add_argument('--orig_t_size', type=int, default=32)
    ...
    parser.add_argument('--drop_path', type=float, default=0.35, metavar='PCT',
    ...
    # Finetuning params
    parser.add_argument('--finetune', default='./weights/ego_rgb_best.pth', help='finetune from checkpoint')
    ...
    # Dataset parameters
    parser.add_argument('--prefix', default='/media/diskE/mali/datasets/Ego', type=str,
                        help='prefix for data')
    parser.add_argument('--depth_prefix', default='/media/diskE/mali/datasets/Ego', type=str,
                        help='prefix for data')
    parser.add_argument('--split', default=' ', type=str, help='split for metadata')
    parser.add_argument('--filename_tmpl', default='{:06}.jpg', type=str, help='file template')
    parser.add_argument('--depth_filename_tmpl', default='{:06}.jpg', type=str, help='file template')
    parser.add_argument('--data_path', default='./dataset_splits/EgoGesture/rgb', type=str,
                        help='dataset path')
    parser.add_argument('--depth_data_path', default='./dataset_splits/EgoGesture/depth', type=str,
                        help='dataset path')
    ...
    parser.set_defaults(fusion=True)
    parser.add_argument('--data_set', default='Ego', choices=[
        'NV', 'ISO', 'FPHA', 'THU', 'Ego'], type=str, help='dataset')
    parser.add_argument('--nb_classes', default=83, type=int,
                        help='NV:25, ISO:249, THU:40, EGO:83, FPHA:45')

    parser.add_argument('--output_dir', default=f"{OUTPUT_DIR}",
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default=f"{LOG_DIR}",
    ...
    parser.add_argument('--no_save_ckpt', action='store_false', dest='save_ckpshort_side_sizet')
...
def main(args, ds_init):
    utils_fusion.init_distributed_mode(args)

    dist.init_process_group('gloo', init_method='file:///tmp/somefile', rank=0, world_size=1)

    if ds_init is not None:
        utils_fusion.create_ds_config(args)
    ...
    seed = args.seed + utils_fusion.get_rank()
    ...
    num_tasks = utils_fusion.get_world_size()
    global_rank = utils_fusion.get_rank()
    ...
    if global_rank == 0 and args.log_dir is not None:
        ...
        log_writer = utils_fusion.TensorboardLogger(log_dir=args.log_dir)
    else:
        log_writer = None
    ...
    if args.finetune:
        ...
        utils_fusion.load_state_dict(model, checkpoint_model, prefix=args.model_prefix)
    ...
    total_batch_size = args.batch_size * args.update_freq * utils_fusion.get_world_size()
    ...
    lr_schedule_values = utils_fusion.cosine_scheduler(
    ...
    wd_schedule_values = utils_fusion.cosine_scheduler(
    ...
    if args.eval:
        ...
        if args.output_dir and utils_fusion.is_main_process():
