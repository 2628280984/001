...
class VisionMamba(nn.Module):
    ...
    def forward(self, rgb, depth, inference_params=None):
        # rgb_input，depth_input：(1, 3, 16, 224, 224)
        rgb = self.forward_features(rgb, inference_params)  # (1, 576)
        depth = self.forward_features(depth, inference_params)  # (1, 576)
...
@register_model
def videomamba_tiny_fusion1(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, 
        embed_dim=192, 
        depth=24, 
...
@register_model
def videomamba_small_fusion1(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, 
        embed_dim=384, 
        depth=24, 
...
@register_model
def videomamba_middle_fusion1(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, 
        embed_dim=576, 
        depth=32, 
...
if __name__ == '__main__':
    model = videomamba_small_fusion1(num_frames=16,num_classes=25).cuda()
