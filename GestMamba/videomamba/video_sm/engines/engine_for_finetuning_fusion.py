...
def train_class_batch(model, samples, samples_depth, targets_rgb, targets_depth, criterion):
    outputs = model(samples, samples_depth)
    loss = criterion(outputs, targets_rgb) + criterion(outputs, targets_depth)
    return loss, outputs
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, samples_depth, targets, _, _) in enumerate(
        ...
        samples_depth = samples_depth.to(device, non_blocking=True)
        targets = targets.to(device, non_blocking=True)
        targets_rgb, targets_depth = targets, targets

        if mixup_fn is not None:
            samples, targets_rgb = mixup_fn(samples, targets_rgb)
            samples_depth, targets_depth = mixup_fn(samples_depth, targets_depth)

        if loss_scaler is None:
            if not no_amp:
                samples = samples.bfloat16() if bf16 else samples.half()
                samples_depth = samples_depth.bfloat16() if bf16 else samples_depth.half()
            loss, output = train_class_batch(
                model, samples, samples_depth, targets_rgb, targets_depth, criterion)
        else:
            with amp_autocast:
                loss, output = train_class_batch(
                    model, samples, samples_depth, targets_rgb, targets_depth, criterion)
...
def validation_one_epoch(data_loader, model, device, amp_autocast, ds=True, no_amp=False, bf16=False, maxk=5):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        videos = batch[0]
        videos_depth = batch[1]
        target = batch[2]
        videos = videos.to(device, non_blocking=True)
        videos_depth = videos_depth.to(device, non_blocking=True)
        ...
        if ds:
            if not no_amp:
                ...
                videos_depth = videos_depth.bfloat16() if bf16 else videos_depth.half()
            output = model(videos, videos_depth)
