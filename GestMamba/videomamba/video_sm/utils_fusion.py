...
def multiple_samples_collate(batch, fold=False):
    ...
    """
    inputs, depth_inputs, labels, video_idx, extra_data = zip(*batch)

    inputs = [item for sublist in inputs for item in sublist]
    depth_inputs = [item for sublist in depth_inputs for item in sublist]
    labels = [item for sublist in labels for item in sublist]
    video_idx = [item for sublist in video_idx for item in sublist]

    inputs, depth_inputs, labels, video_idx, extra_data = (
        default_collate(inputs),
        default_collate(depth_inputs),
        default_collate(labels),
        default_collate(video_idx),
        default_collate(extra_data),
    )
    if fold:
        return [inputs, depth_inputs], labels, video_idx, extra_data
    else:
        return inputs, depth_inputs, labels, video_idx, extra_data
