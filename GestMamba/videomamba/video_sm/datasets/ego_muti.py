...
class mutiEgoRawFrameClsDataset(Dataset):
    """Load your own raw frame classification dataset."""

    def __init__(self, anno_path, depth_anno_path, prefix='', depth_prefix='', split=' ', mode='train', clip_len=8,
                 ...
                 test_num_crop=3, filename_tmpl='{:06}.jpg', depth_filename_tmpl='{:06}.jpg', dataset='NV', args=None):
        self.anno_path = anno_path
        self.depth_anno_path = depth_anno_path
        self.prefix = prefix
        ...
        depth_cleaned = pd.read_csv(self.depth_anno_path, header=None, delimiter=self.split)
        self.dataset_samples = list(cleaned.values[:, 0].astype('str'))
        self.depth_dataset_samples = list(depth_cleaned.values[:, 0].astype('str'))
        self.label_array = list(cleaned.values[:, 1])
        self.start_frames = list(cleaned.values[:, 2].astype(int))
        self.end_frames = list(cleaned.values[:, 3].astype(int))
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            depth_sample = self.depth_dataset_samples[index]

            start_frames = self.start_frames[index]
            end_frames = self.end_frames[index]
            total_frame = end_frames - start_frames

            sample_id = "_".join(sample.split("/")[:2]) + "_" + sample.split("/")[-1][-1] + '_' + str(
                start_frames) + "_" + str(end_frames)
            depth_sample_id = "_".join(depth_sample.split("/")[:2]) + "_" + depth_sample.split("/")[-1][-1] + '_' + str(
                start_frames) + "_" + str(end_frames)
            assert sample_id == depth_sample_id, "RGB和深度视频不对应！"

            frame_indices = self.get_frame_indices(total_frame)

            buffer = self.load_frame_with_indices(sample, frame_indices, start_frames, self.filename_tmpl)
            depth_buffer = self.load_depth_frame_with_indices(depth_sample, frame_indices, start_frames, self.depth_filename_tmpl)

            # buffer = self.load_frame(sample, total_frame, sample_rate_scale=scale_t)  # T H W C、
            # depth_buffer = self.load_frame(depth_sample, total_frame, sample_rate_scale=scale_t)  # T H W C

            if len(buffer) == 0 or len(depth_buffer) == 0:
                while len(buffer) == 0 or len(depth_buffer) == 0:
                    warnings.warn(
                    ...
                    frame_indices = self.get_frame_indices(total_frame)
                    buffer = self.load_frame_with_indices(sample, frame_indices, start_frames, self.filename_tmpl)
                    depth_buffer = self.load_depth_frame_with_indices(depth_sample, frame_indices, start_frames,
                                                                      self.depth_filename_tmpl)
                    # buffer = self.load_frame(sample, total_frame, sample_rate_scale=scale_t)  # T H W C、
                    # depth_buffer = self.load_frame(depth_sample, total_frame, sample_rate_scale=scale_t)  # T H W C
            ...
            if args.num_sample > 1:
                ...
                for _ in range(args.num_sample):
                    ...
                    new_depth_frames = self._aug_frame(depth_buffer, args)
                    label = self.label_array[index] - 1
                    frame_list.append(new_frames)
                    depth_frame_list.append(new_depth_frames)
                    label_list.append(label)
                    index_list.append(index)
                return frame_list, depth_frame_list, label_list, index_list, {}
            else:
                buffer = self._aug_frame(buffer, args)
                depth_buffer = self._aug_frame(depth_buffer, args)

            return buffer, depth_buffer, self.label_array[index]-1, index, {}
            ...
            # depth_buffer = self.load_frame(depth_sample, total_frame)
            ...
            depth_buffer = self.data_transform(depth_buffer)

            return buffer, depth_buffer, self.label_array[index]-1, sample_id
    ...
    def _aug_frame(
        ...
        buffer = spatial_sampling(
            ...
            random_horizontal_flip=False if args.data_set =='Ego' else True,
    ...
    def get_frame_indices(self, num_frames):
        """Generate frame indices for RGB and depth."""
        ...
        return list(np.array(all_index))

    def load_frame_with_indices(self, sample, frame_indices, start_frames, filename_tmpl):
        """Load frames using given frame indices."""
        fname = os.path.join(self.prefix, sample)
        imgs = []
        for idx in frame_indices:
            frame_fname = os.path.join(fname, filename_tmpl.format(idx + start_frames))
            if "s3://" in fname:
    ...
    def load_depth_frame_with_indices(self, sample, frame_indices, start_frames, filename_tmpl):
        """Load frames using given frame indices."""
        fname = os.path.join(self.depth_prefix, sample)
        imgs = []
        for idx in frame_indices:
            frame_fname = os.path.join(fname, filename_tmpl.format(idx + start_frames))
            if "s3://" in fname:
    ...
    def load_frame(self, sample, num_frames, filename_tmpl, sample_rate_scale=1):
        ...
        for idx in all_index:
            frame_fname = os.path.join(fname, filename_tmpl.format(idx + 1))
            if "s3://" in fname:
                img_bytes = self.client.get(frame_fname)
            else:
                with open(frame_fname, 'rb') as f:
...
class mutiEgoVideoClsDataset(Dataset):
