...
class EgoRawFrameClsDataset(Dataset):
    ...
    def __init__(self, anno_path, prefix='', split=' ', mode='train', clip_len=8,
                 ...
                 test_num_crop=3, filename_tmpl='img_{:05}.jpg', dataset='NV', args=None):
        ...
        if (mode == 'train'):
            ...
            self.test_seg = []
            self.test_dataset = []
            self.test_start_frames = []
            self.test_end_frames = []
            ...
            for ck in range(self.test_num_segment):
                for cp in range(self.test_num_crop):
                    for idx in range(len(self.label_array)):
                        ...
                        self.test_start_frames.append(self.start_frames[idx])
                        self.test_end_frames.append(self.end_frames[idx])
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            buffer = self.load_frame(sample, start_frames, end_frames, sample_rate_scale=scale_t)  # T H W C
            if len(buffer) == 0:
                while len(buffer) == 0:
                    warnings.warn(
                    ...
                    buffer = self.load_frame(sample, start_frames, end_frames, sample_rate_scale=scale_t)
            ...
            return buffer, self.label_array[index]-1, index, {}
            ...
            sample_id = "_".join(sample.split("/")[:2]) + "_" + sample.split("/")[-1][-1] + '_' + str(start_frames) + "_" + str(end_frames)

            buffer = self.load_frame(sample, start_frames, end_frames)
            ...
            return buffer, self.label_array[index]-1, sample_id

        elif self.mode == 'test':
            sample = self.test_dataset[index]

            chunk_nb, split_nb = self.test_seg[index]
            start_frames = self.test_start_frames[index]
            end_frames = self.test_end_frames[index]
            ...
            buffer = self.data_transform(buffer)
            return buffer, self.test_label_array[index]-1, sample_id, \
                   chunk_nb, split_nb
        else:
    ...
    def load_frame(self, sample, start_frame, end_frame, sample_rate_scale=1):
        """根据起始帧和结束帧加载视频帧"""
        fname = sample
        fname = os.path.join(self.prefix, fname)
        num_frames = end_frame - start_frame  # 加载的总帧数
        ...
        if self.mode == 'test':
            ...
            for idx in all_index:
                frame_fname = os.path.join(fname, self.filename_tmpl.format(start_frame + idx))  # 加载指定帧的文件
                if "s3://" in fname:
        ...
        imgs = []
        for idx in all_index:
            frame_fname = os.path.join(fname, self.filename_tmpl.format(start_frame + idx))
            if "s3://" in fname:
                img_bytes = self.client.get(frame_fname)
            else:
                with open(frame_fname, 'rb') as f:
...
class EgoVideoClsDataset(Dataset):
