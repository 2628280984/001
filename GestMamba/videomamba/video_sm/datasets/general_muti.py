...
class MMRawFrameClsDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            total_frame = self.total_frames[index] - 1
            if self.dataset == 'THU':
                sample_id = sample.replace("RGB_", "")
                depth_sample_id = depth_sample.replace("D_", "")
                assert sample_id == depth_sample_id, "RGB和深度视频不对应！"
            elif self.dataset == 'NV':
                sample_id = sample.replace("/", "_")
                depth_sample_id = depth_sample.replace("/", "_")
                assert sample_id == depth_sample_id, "RGB和深度视频不对应！"
            elif self.dataset == 'ISO':
                sample_id = sample.replace("/", "_").replace("M_", "")
                depth_sample_id = depth_sample.replace("/", "_").replace("K_", "")
                assert sample_id == depth_sample_id, "RGB和深度视频不对应！"
            elif self.dataset == 'FPHA':
                sample_id = '_'.join(sample.split('/')[:-1])
                depth_sample_id = '_'.join(depth_sample.split('/')[:-1])
                assert sample_id == depth_sample_id, "RGB和深度视频不对应！"

            frame_indices = self.get_frame_indices(total_frame)

            buffer = self.load_frame_with_indices(sample, frame_indices, self.filename_tmpl)
            depth_buffer = self.load_depth_frame_with_indices(depth_sample, frame_indices, self.depth_filename_tmpl)
            ...
            return buffer, depth_buffer, self.label_array[index], index, {}
            ...
            return buffer, depth_buffer, self.label_array[index], sample_id
    ...
    def _aug_frame(
        ...
        buffer = spatial_sampling(
            ...
            random_horizontal_flip=False if args.data_set in ['NV', 'ISO', 'FPHA', 'THU'] else True,
    ...
    def load_frame_with_indices(self, sample, frame_indices, filename_tmpl):
    ...
    def load_depth_frame_with_indices(self, sample, frame_indices, filename_tmpl):
        """Load frames using given frame indices."""
...
class MMVideoClsDataset(Dataset):
